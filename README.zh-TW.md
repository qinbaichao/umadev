...
- **把工程規範帶進底座。** 418 份精選知識庫檔案（商業級工程規範、設計規則，以及你現有程式碼的結構圖）編譯進二進位檔，在每個工作回合注入底座——零設定，啟動時解壓到 `~/.umadev/knowledge`，之後無需重複下載。
- **從每次執行裡學。** 反覆犯的錯會被記錄下來（`.umadev/reflections/`），在後續工作裡召回，避免同一個坑再踩一遍。
...
| `umadev-knowledge` | 知識檢索 | BM25、chunk、tokenizer、可選 vector、`repo_map` 程式碼結構圖 |
