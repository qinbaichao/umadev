...
export const i18n = {
  zh: {
    ...
    hero: {
      ...
      titleHi: "真正的项目总监",
      title2: " Agent",
      sub: "借底座大脑判断你这句话：闲聊就直接回，真要构建就自动召集角色团队评审、套上设计系统与知识库、留下交付证明。UmaDev 驱动你已登录的 Claude Code / Codex CLI / OpenCode，一个常驻持续会话——启动即预载，不再扛每条消息的冷启动。",
      cta1: "快速开始",
      cta2: "阅读文档",
      copy: "复制",
      copied: "已复制",
    ...
    flow: {
      ...
      layers: [
        ...
        { k: "项目总监 Agent", d: "借底座大脑路由这一步，拥有并驱动可见计划，按步调度角色团队。" },
  ...
  en: {
    ...
    hero: {
      ...
      title2: " agent",
      sub: "The borrowed brain judges your message: chit-chat just answers; a real build automatically convenes the role team, layers on the design system and knowledge base, and leaves a delivery proof. UmaDev drives the Claude Code / Codex CLI / OpenCode you already logged into — one resident session, pre-warmed at launch so there's no per-message cold start.",
      cta1: "Get started",
      cta2: "Read the docs",
      copy: "Copy",
      copied: "Copied",
    ...
    flow: {
      ...
      layers: [
        ...
        { k: "Project director agent", d: "Borrows the brain to route the turn, owns and drives a visible plan, schedules the role team step by step." },
...
export const releases = {
  zh: [
    { ver: "1.0.7", date: "2026-06-24", current: true, title: "借脑路由 + 真实构建即全套系统 + 终端渲染对标", changes: [["新增", "借脑智能路由:默认聊天界面借底座大脑判断每句话——闲聊 / 解释 / 小改 / 调试 / 构建,底座自己的模型说了算(权威),大脑不可达则走最轻路径,绝不硬编码关键词;底座还会「以行动判断」——首次写入工作区文件就把聊天回合反应式升格为构建"], ["新增", "取消「聊天 vs /run」分叉:触发重型系统的是「真实构建」本身,不是某条命令。聊天里随手提的构建,与 /run 享有同一套旗舰系统;/run、/goal 仍是显式入口,不再是独立代码路径"], ["新增", "统一常驻系统:每次真实构建(聊天升格或 /run)都自动拥有——设计系统 / 反 AI-slop 法(每个干活回合常驻,静态法无延迟)、构建后治理 + 设计 slop 扫描、角色团队评审(PM / 架构 / UI-UX / 前端 / 后端 / QA / 安全,只读 fork,建议性)、知识库摘要、自进化(记踩坑、把教训召回进返工)。Fast 构建召集精简 UI 团队(设计+前端+QA),deliberate 构建召集全员"], ["新增", "/goal 命令登陆三底座:`/goal <目标>` 驱动目标导向构建,持续让借来的大脑工作直到目标达成,带完整统一系统;claude / codex / opencode 均支持原生持续 /goal 模式(UMADEV_NO_GOAL_MODE=1 可退出)"], ["新增", "知识库内置进二进制:完整语料(418 份)随包内置,首次运行自动解压到 ~/.umadev/knowledge——商业工程规范零配置下发到每个用户项目,不再是用户机上的空语料"], ["改进", "持续会话免冷启动:聊天跑在一个常驻底座会话上,启动时预加载(底座 + MCP + 固件只加载一次),首次回复不再扛旧的每条消息 30-60 秒冷启动"], ["改进", "终端渲染对标 Claude Code:真实 Markdown(CJK 对齐表格 / 标题 / 嵌套列表 / 粗斜体 / 链接显 URL / 任务勾选框 / 分语言高亮代码块);文件 Write/Edit 渲染成实时 diff 卡,词级高亮(只点亮改动 token)、行号边栏、虚线框;干净的工具调用行(只读工具合并 + 长输出折叠,Ctrl+R 展开);构建完成卡列出改动文件 + 运行命令 + 自动浮出可点的预览地址(http://localhost:PORT);流式打磨(稳定前缀缓存防卡顿 / 粘底 / 统一带微光的 spinner)"]] },
  ...
  en: [
    { ver: "1.0.7", date: "2026-06-24", current: true, title: "Brain-driven routing + a real build earns the full systems + terminal rendering at parity", changes: [["Added", "Brain-driven routing: the default chat surface borrows the base's brain to judge every turn — chat / explain / quick-edit / debug / build. The base's own model decides, authoritatively; if the brain is unreachable it takes the lightest path, never a keyword guess. The base also judges by acting — the first workspace file write reactively promotes a chat turn into a build"], ["Added", "No more chat-vs-/run split: the trigger for the heavy systems is a real build itself, not a typed command. A build mentioned in chat earns the same flagship systems as /run; /run and /goal stay as explicit entries, not separate code paths"], ["Added", "Unified always-on systems: every real build (chat-promoted or /run) automatically gets the design-system / anti-AI-slop law (always-on every work turn, a static law with no latency cost), a post-build governance + design-slop scan, the critic team review (PM / architect / UI-UX / frontend / backend / QA / security — read-only forks, advisory), the curated knowledge digest, and self-evolution (records pitfalls, recalls lessons into the rework). A Fast build convenes a minimal UI team (designer + frontend + QA); a deliberate build the full roster"], ["Added", "/goal command on all three bases: `/goal <objective>` drives a goal-driven build that keeps the borrowed brain working until the objective is met, with the full unified system; claude / codex / opencode all support a native persistent /goal mode (UMADEV_NO_GOAL_MODE=1 opts out)"], ["Added", "Knowledge bundled into the binary: the full curated corpus (418 files) is embedded and auto-extracted to ~/.umadev/knowledge on first run, so commercial engineering standards reach every user project with zero config — no longer an empty corpus on a user machine"], ["Improved", "Persistent-session speed: chat runs on one resident base session pre-loaded at launch (base + MCP + firmware loaded once) → the first reply no longer pays the old 30-60s per-message cold start"], ["Improved", "Terminal rendering at Claude-Code parity: real Markdown (CJK-safe aligned tables, headings, nested lists, bold/italic, links that surface their URL, task-list checkboxes, per-language highlighted code); a file Write/Edit renders as a real-time diff card with word-level highlighting (only changed tokens light up), a line-number gutter and a dashed frame; clean tool-call rows (read-only tools merged, long output folded, Ctrl+R to expand); a build-completion card with changed files + run command + an auto-surfaced clickable preview URL (http://localhost:PORT); streaming polish (stable-prefix cache, sticky-to-bottom, a unified shimmer spinner)"]] },
